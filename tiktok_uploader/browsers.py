...
def get_service(name: str = "chrome"):
    ...
    if _clean_name(name) in services:
        if name == "chrome" or name == "chromium":
            return ChromeService('/usr/bin/chromedriver')
...
def chrome_defaults(
    *args, headless: bool = False, proxy: dict = None, **kwargs
    ...
    options.add_argument("driver_path=/usr/bin/chromedriver")
...
def chromium_defaults(
    *args, headless: bool = False, proxy: dict = None, **kwargs
) -> ChromeOptions:
    """
    Creates Chromium with default options
    """
    options = ChromeOptions()
...
services = {
    ...
    "chromium": lambda: ChromeService(ChromeDriverManager().install()),
}
